...
def parse_dataset(sub_list, train_X, train_subs, train_label):
    ...
    for sub in sub_list:
        sub_flags = train_subs==sub
        for label in range(1, num_labels+1):
            label_flags = train_label==label
            data_dict[sub].append(train_X[sub_flags & label_flags])
        data_dict[sub] = np.concatenate(data_dict[sub], axis=0)
    return data_dict
...
def run(args, ch_ratios=1, manual_ch:list=None, onsets:dict=None, saveroot:str='/home/yainoue/meg2image/results/20230523_corr_corr_resample120'):
    ...
    corr_of_corr1 = run_normalize_trial(sbj_data_dict[sub_list[0]], sbj_data_dict[sub_list[1]], saveroot, 's1d1_norm_unit') # 0.01078 # 0.01068 # 0.03266 (2-13 Hz) #  0.0160 # 0.032 # 0.021 # 0.21(30-40) # 0.023 # 0.024(30-60) # 0.0265(30-70) # 0.028(60-70) # 0.043(60-100) # 0.046

    print('{} vs{}: '.format(sub_list[0], sub_list[1]), corr_of_corr1)
